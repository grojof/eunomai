...
- [x] 1.1 Draft `agents/codebase-cartographer.md` with `agent-creator`, then fit it to the house style:
...
- [x] 1.2 Draft `agents/coherence-auditor.md` likewise: one-shot diagnostic comparing doc claims vs code/versions,
...
- [x] 1.3 Sanity-check both agents with `plugin-validator` / `skill-reviewer` where useful; keep them lean.

## 2. Wire the delegations

- [x] 2.1 `skills/eunomai-onboard/SKILL.md`: in the analyze step, delegate comprehension to the cartographer and
      (for existing projects) a one-shot coherence audit to the auditor — read-only, human confirms.
- [x] 2.2 `skills/eunomai-living-docs/SKILL.md`: delegate the architecture diagram to the cartographer and
      stale-doc detection to the auditor — read-only, suggestion-only.

## 3. Sync specs + verify the gate

- [x] 3.1 Sync the `onboard` and `living-docs` deltas into the main specs.
- [x] 3.2 Run `openspec validate --specs` — all specs pass.
- [x] 3.3 Run `node tools/dist/cli.cjs docs-check` and `provenance-check` — both exit 0 (agents need no registry
