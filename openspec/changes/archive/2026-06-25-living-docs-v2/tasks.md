...
- [x] 1.1 Extend `tools/src/docs.ts` to parse each in-scope `docs/` page's YAML frontmatter (reuse the `yaml`
...
- [x] 1.2 Extend `tools/test/docs.test.ts` with cases: missing frontmatter, unknown `type`, empty
      `title`/`description`, and a valid page. Keep prose/coherence explicitly out of scope.
- [x] 1.3 `cd tools && npm run typecheck && npm run lint && npm test && npm run build`; commit the rebuilt bundle.

## 2. Rewrite the v2 standard

- [x] 2.1 Rewrite `docs/reference/living-docs.md` to the v2 model: Diátaxis-as-lens via `type`; the frontmatter
...
- [x] 2.2 Update `skills/eunomai-living-docs/SKILL.md` to the v2 standard (frontmatter, the map, `type` lens,
      the two-layer guarantee).

## 3. Pilot on eunomai's own docs

- [x] 3.1 Add v2 frontmatter to every page under `docs/guides/`, `docs/reference/`, `docs/explanation/`
      (`type`/`title`/`description` required; `tags` + sensible optionals).
- [x] 3.2 Delegate to the `codebase-cartographer` for an "at a glance" architecture diagram (Mermaid/C4) and
      place it in the README.
- [x] 3.3 Rewrite `README.md` as the routable **map**: at-a-glance summary · the diagram · quickstart · the
      surface-organized index. Keep `docs/decisions/` excluded from the index.

## 4. Sync spec + verify the gate

- [x] 4.1 Sync the `living-docs` delta into `openspec/specs/living-docs/spec.md` and update its Purpose to v2.
- [x] 4.2 Run `openspec validate --specs` — all specs pass.
- [x] 4.3 Run the full gate: `docs-check` (now incl. frontmatter) + `provenance-check` exit 0; hooks tests pass;
