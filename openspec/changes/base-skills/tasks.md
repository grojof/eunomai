...
- [x] 1.1 Add `skills/eunomai-secure-coding/SKILL.md` — actionable directives across the **OWASP Top 10** (+
...
- [x] 2.1 Add `skills/eunomai-dependency-upgrade/SKILL.md` — add/upgrade hygiene: lockfile, **SCA/CVE scan
...
- [x] 3.1 Update `skills/eunomai-living-docs/SKILL.md` and `docs/reference/living-docs.md` — guide **Mermaid**
...
- [x] 4.1 Add `eunomai-secure-coding` and `eunomai-dependency-upgrade` to `skills/eunomai-skills-audit.md`
  (`origin: authored`) (req: *Base skills are authored and audited*).
- [x] 4.2 Update `AGENTS.md` with a **base skills** note (the admission filter + the two skills) and re-project
  to `CLAUDE.md` / `copilot-instructions.md`.

## 5. Validation gate

- [x] 5.1 `node projection/dist/cli.cjs provenance-check` exits 0 (registry covers all skills, incl. the two new ones).
...
- [x] 5.4 Validate the change: `openspec validate base-skills --strict`.
- [x] 5.5 Sanity (no projection code changed): `cd projection && npm run typecheck && npm run lint && npm test`.
