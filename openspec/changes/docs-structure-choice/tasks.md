...
- [x] 1.1 In `skills/eunomai-living-docs/SKILL.md`, add a "Choosing the structure" step: assess size/shape,
...
- [x] 1.2 Add the **hard rule + anti-pattern flag**: folders never by Diátaxis type; if `guides/`/`reference/`/
...
- [x] 2.1 In `skills/eunomai-onboard/SKILL.md`, the establish-docs step proposes the structure the same way
      (2–3 options, recommended default, confirm) rather than assuming a layout.

## 3. Sync specs + verify the gate

- [x] 3.1 Sync the `living-docs` and `onboard` deltas into the main specs.
- [x] 3.2 Run `openspec validate --specs` — all specs pass.
- [x] 3.3 Run `docs-check` + `provenance-check` — both exit 0 (no behaviour/gate change). No code changed.
