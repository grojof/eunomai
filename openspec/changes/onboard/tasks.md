...
- [ ] 1.1 Add `skills/eunomai-onboard/SKILL.md` — the cold-start playbook: **analyze** the project + gather
...
- [ ] 1.2 Add `skills/eunomai-onboard/PROVENANCE.md` (`origin: authored`) so `provenance-check` stays green
  (req: *onboard carries provenance*).

## 2. Docs

- [ ] 2.1 Add `docs/onboard.md` (project-doc: the cold-start orchestrator, what it seeds, the hand-off) and
  index it in the README (keeps `docs-check` green; dogfoods living-docs).
- [ ] 2.2 Document the connector/bootstrap axis in `AGENTS.md` — onboard as the cold-start orchestrator that
  seeds and steps aside — then re-project to `CLAUDE.md` / `copilot-instructions.md`.

## 3. Validation gate

- [ ] 3.1 `node projection/dist/cli.js docs-check` exits 0 (new page indexed).
- [ ] 3.2 `node projection/dist/cli.js provenance-check` exits 0 (three skills now, all with provenance).
- [ ] 3.3 Re-project and verify idempotency: `compile` then `compile --check` (zero drift).
- [ ] 3.4 Validate the change: `openspec validate onboard --strict`.
- [ ] 3.5 Sanity (no code changed, but confirm nothing broke): `cd projection && npm run typecheck && npm run lint && npm test`.
