...
- [ ] 1.1 Update `skills/eunomai-onboard/SKILL.md`: add the **structured-interview** technique to the
...
- [ ] 1.2 Update the skill so **creating docs from scratch** produces byproduct artifacts via the interview:
...
- [ ] 2.1 Update `skills/eunomai-living-docs/SKILL.md`: reference the structured-interview technique to recover
...
- [ ] 3.1 Update `docs/reference/onboard.md` — the structured interview in input-gathering and from-scratch doc
  creation (ADRs + glossary byproducts); keep `docs-check` green.
- [ ] 3.2 Update `docs/reference/living-docs.md` — the interview technique for recovering thin/missing docs.
- [ ] 3.3 If the onboard/living-docs sections of `AGENTS.md` change, update them and **re-project**
...
- [ ] 4.3 `compile` then `compile --check` — zero drift.
- [ ] 4.4 Validate the change: `openspec validate interview-extraction --strict`.
- [ ] 4.5 No projection/checker code changed; if any did, run `cd projection && npm run typecheck && npm run lint && npm test`.
