...
- [x] 1.1 Rework `projection/src/provenance.ts`: define a zod schema for the `eunomai-skills-audit.md`
...
- [x] 1.2 Update the `provenance-check` CLI: exit non-zero on uncovered/invalid; print a **gaps** section
  (e.g. `unpinned`) for action; success message reports skills covered (req: *provenance-check*).
- [x] 1.3 Rewrite `projection/test/provenance.test.ts` for the registry model: covered → ok; uncovered skill
...
- [x] 2.1 Update `skills/eunomai-skill-finder/SKILL.md`: maintain the **`eunomai-skills-audit.md` registry**
...
- [x] 3.1 Delete eunomai's three `PROVENANCE.md` sidecars; add `skills/eunomai-skills-audit.md` covering
  `eunomai-living-docs`, `eunomai-skill-finder`, `eunomai-onboard` (origin `authored`) (req: *eunomai
  dogfoods provenance*).

## 4. Docs + authored source

- [x] 4.1 Update `docs/reference/skill-finder.md`: the consolidated registry (not sidecars), the multi-root
  check, and gap reporting.
- [x] 4.2 Update the `AGENTS.md` Skills section (registry, not per-skill sidecar) and re-project to
  `CLAUDE.md` / `copilot-instructions.md`.

## 5. Validation gate

- [x] 5.1 Rebuild the bundle, then `node projection/dist/cli.cjs provenance-check` exits 0 (registry covers
  the three own skills).
- [x] 5.2 `node projection/dist/cli.cjs docs-check` exits 0.
- [x] 5.3 Re-project and verify idempotency: `cli.cjs compile` then `compile --check` (zero drift).
- [x] 5.4 Validate the change: `openspec validate skills-audit-registry --strict`.
- [x] 5.5 Run the gate (code changed): `cd projection && npm run typecheck && npm run lint && npm test`.
